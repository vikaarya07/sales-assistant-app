            ...
            <flux:modal name="two-factor-setup-modal" class="max-w-md md:min-w-md" @close="closeModal"
                ...
                <div class="space-y-6">
                    <div class="flex flex-col items-center space-y-4">
                        <div
                            class="p-0.5 w-auto rounded-full border border-indigo-100 dark:border-indigo-900 bg-white dark:bg-zinc-800 shadow-sm">
                            <div
                                class="p-2.5 rounded-full border border-violet-200 dark:border-violet-800 overflow-hidden bg-violet-50 dark:bg-violet-950 relative">
                                <div
                                    class="flex items-stretch absolute inset-0 w-full h-full divide-x [&>div]:flex-1 divide-violet-200 dark:divide-violet-800 justify-around opacity-50">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <div></div>
                                    @endfor
                                </div>
                                ...
                                <div
                                    class="flex flex-col items-stretch absolute w-full h-full divide-y [&>div]:flex-1 inset-0 divide-fuchsia-200 dark:divide-fuchsia-800 justify-around opacity-40">
                                    @for ($i = 1; $i <= 5; $i++)
                                        <div></div>
                                    @endfor
                                </div>
                    ...
                    @else
                        ...
                        <div class="flex justify-center">
                            <div
                                class="relative w-64 overflow-hidden border rounded-lg border-violet-200 dark:border-violet-800 aspect-square">
                            @empty($qrCodeSvg)
                                <div
                                    class="absolute inset-0 flex items-center justify-center bg-white dark:bg-zinc-800 animate-pulse">
                                    <flux:icon.loading />
                                </div>
                            @else
                    ...
                    <div class="space-y-4">
                        ...
                        <div class="flex items-center space-x-2" x-data="{
                            ...
                            <div
                                class="flex items-stretch w-full border rounded-xl border-violet-200 dark:border-violet-800">
                            @empty($manualSetupKey)
                                <div
                                    class="flex items-center justify-center w-full p-3 bg-violet-50 dark:bg-violet-950">
                            ...
                            @else
                                ...
                                <button @click="copy()"
                                    ...
                                    <flux:icon.document-duplicate x-show="!copied" variant="outline"></flux:icon>
                                        ...
                                        <flux:icon.check x-show="copied" variant="solid" class="text-violet-500">
                                            </flux:icon>
                                </button>
                            @endempty
                        </div>
...
@if ($canManagePasskeys)
    <section class="mt-12">
        ...
        <div class="mt-6 flex flex-col w-full mx-auto space-y-6 text-sm" wire:cloak>
            <div class="border rounded-lg border-indigo-200/70 dark:border-indigo-800/60 overflow-hidden">

                @forelse ($passkeys as $passkey)
                    <div
                        class="flex items-center justify-between p-4 {{ !$loop->last ? 'border-b border-violet-200/60 dark:border-violet-800/50' : '' }}">

                        <div class="flex items-center gap-4">
                            <div
                                class="flex size-10 shrink-0 items-center justify-center rounded-xl bg-linear-to-br from-indigo-100 via-violet-100 to-fuchsia-100 dark:from-indigo-950 dark:via-violet-950 dark:to-fuchsia-950">
                                <flux:icon.key class="size-5 text-violet-500 dark:text-violet-300" />
                            </div>
                            ...
                            <div class="space-y-1">
                                ...
                                <p class="text-violet-600/70 dark:text-violet-300/70 text-xs">
                ...
                @empty
                    <div class="p-8 text-center">
                        <div
                            class="mx-auto mb-4 flex size-14 items-center justify-center rounded-2xl bg-linear-to-br from-indigo-100 via-violet-100 to-fuchsia-100 dark:from-indigo-950 dark:via-violet-950 dark:to-fuchsia-950">
                            <flux:icon.key class="size-7 text-violet-400 dark:text-violet-300" />
                        </div>
